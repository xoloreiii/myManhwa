    ...
    <div className="card flex flex-col">
      <div className="relative aspect-[2/3] bg-[#1a1835] overflow-hidden">
        ...
        <div className="absolute top-2 right-2" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-8 z-50 w-32 rounded-xl shadow-2xl shadow-black/50 overflow-hidden py-1"
              style={{ background: '#251d3d', border: '1px solid rgba(185,166,245,0.35)' }}>
              <button onClick={() => { onEdit(manhwa); setMenuOpen(false) }}
      ...
      <div className="p-3 space-y-1.5">
        <span className={isCompleted ? 'badge-completed' : 'badge-ongoing'}>
        ...
        <p className="text-[11px] text-slate-500">
          Last read: Ch. {manhwa.chapter}{isCompleted ? ' (Finished)' : ''}
        </p>
        <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
          <span className={CATEGORY_BADGE[manhwa.category]}>{manhwa.category}</span>
          {genreList.slice(0, 2).map(g => (
            <span key={g} className="text-[10px] font-medium px-2 py-0.5 rounded-md text-slate-400"
              style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}>
              {g}
            </span>
          ))}
          {genreList.length > 2 && <span className="text-[10px] text-slate-600">+{genreList.length - 2}</span>}
        </div>
        {flag && (
