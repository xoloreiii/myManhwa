...
export default function ManhwaForm({ manhwa, onClose, onSuccess }) {
  ...
  const [form, setForm] = useState({
    title: manhwa?.title ?? '', category: manhwa?.category ?? 'NL',
    chapter: manhwa?.chapter ?? '', status: manhwa?.status ?? 'ongoing',
  ...
  const addGenre = useCallback(() => {
    ...
    if (!val || genres.includes(val)) { setGenreInput(''); return }
    setGenres(prev => [...prev, val]); setGenreInput('')
  }, [genreInput, genres])

  const removeGenre = (g) => setGenres(prev => prev.filter(x => x !== g))
  const handleGenreKeyDown = (e) => {
    if (e.key === 'Enter') { e.preventDefault(); addGenre() }
    if (e.key === 'Backspace' && !genreInput && genres.length > 0) setGenres(prev => prev.slice(0, -1))
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = { title: form.title.trim(), category: form.category, chapter: Number(form.chapter), status: form.status, genres, character_flag: form.character_flag || null, poster_url: posterUrl }
      let error
      ...
      <div className="w-full max-w-md rounded-2xl shadow-2xl overflow-hidden"
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="relative w-full h-40 rounded-xl border-2 border-dashed border-white/10 hover:border-purple-500/50 transition-colors cursor-pointer overflow-hidden group"
              ...
              {posterPreview ? (
                ...
                <div className="flex flex-col items-center justify-center h-full gap-2 text-slate-600">
                  <svg className="w-9 h-9" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
          ...
          <div>
            ...
            <input type="text" name="title" value={form.title} onChange={handleChange} placeholder="e.g. Solo Leveling" className="input-field" required />
          </div>
          {/* Category & Chapter */}
          ...
          {/* Genre tags */}
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Genres</label>
            <div className="flex flex-wrap gap-1.5 p-2 rounded-xl border border-white/10 min-h-[42px] cursor-text"
              style={{ background: 'rgba(255,255,255,0.04)' }} onClick={() => document.getElementById('genre-input')?.focus()}>
              {genres.map(g => (
                <span key={g} className="flex items-center gap-1 bg-purple-900/50 border border-purple-700/50 text-purple-200 text-xs font-medium px-2 py-1 rounded-lg">
                  {g}<button type="button" onClick={() => removeGenre(g)} className="text-purple-400 hover:text-white ml-0.5 transition-colors leading-none">×</button>
                </span>
              ))}
              <input id="genre-input" type="text" value={genreInput} onChange={e => setGenreInput(e.target.value)}
                onKeyDown={handleGenreKeyDown} onBlur={addGenre}
                placeholder={genres.length === 0 ? 'Type genre and press Enter...' : ''}
                className="bg-transparent outline-none text-sm text-slate-200 placeholder-slate-600 min-w-[140px] flex-1" />
            </div>
            <p className="text-[11px] text-slate-600 mt-1">Press Enter or click outside to add each genre</p>
          </div>
          {/* Character Flag */}
          <div>
            ...
            {form.character_flag && (
              <button type="button" onClick={() => setForm(p => ({ ...p, character_flag: null }))}
                className="text-[11px] text-slate-600 hover:text-slate-400 mt-1.5 transition-colors">Clear selection</button>
            )}
          </div>
          {/* Status */}
          <div>
              ...
              {STATUSES.map(s => (
                <label key={s.value}
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border cursor-pointer transition-all text-sm font-medium
                    ${form.status === s.value ? 'border-purple-500/60 bg-purple-600/20 text-white' : 'text-slate-500 hover:text-slate-300'}`}
