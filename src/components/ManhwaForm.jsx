        ...
        >
          <svg
            className={`w-7 h-7 transition-colors ${
              star <= (hovered || value) ? 'text-yellow-400 fill-yellow-400' : 'text-slate-600 fill-slate-700'
            }`}
            viewBox="0 0 24 24"
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              {STATUSES.map(s => (
                ...
                >
                  ...
                  <span>{s.label}</span>
                  {form.status === s.value && (
                    <span className="ml-auto w-1.5 h-1.5 rounded-full bg-current opacity-70 flex-shrink-0" />
                  )}
                </label>
              ))}
            </div>
